...
### 2b. Protocolo de desatasco (consulta a Kimi)

Como cuando pides ayuda a un compañero. Invoca a Kimi con el contexto completo del bloqueo (timeout amplio, ≥ 600000 ms):

```bash
kimi --prompt "Lee docs/roles/consultor-kimi.md. Estoy bloqueado en la Tarea N del plan. Síntoma (salida de la validación): <pégala>. Qué he intentado: <resumen de los 4 intentos>. Diagnostica y propón un arreglo concreto."
```

Aplica tú la propuesta (Kimi nunca toca los archivos) y reintenta: **máximo 2 intentos asistidos**, pudiendo consultar de nuevo entre ellos. Si tras ellos la validación sigue en rojo → condición de parada **B** (§5).
...
Si el bot `chatgpt-codex-connector[bot]` está activo en el repo, revisa cada PR automáticamente al abrirlo. **Antes de invocar a Kimi**, espera su revisión y atiéndela:
...
### 4. Solicitar revisión a Kimi

Invócala como subproceso (usa un **timeout amplio**, ≥ 600000 ms):

```bash
kimi --prompt "Lee docs/roles/revisor-kimi.md y revisa el PR #N de este repo. Termina tu salida con una línea que sea exactamente 'VEREDICTO: APROBADO' o 'VEREDICTO: CAMBIOS'."
```

- **`VEREDICTO: APROBADO`** → Kimi ya ha fusionado el PR. Sincroniza (`git checkout main && git pull`) y pasa a la tarea N+1.
...
- **B)** La validación sigue en rojo tras 4 intentos propios + 2 intentos asistidos por la consulta a Kimi (§2b).
- **C)** 2 fallos consecutivos al invocar a Kimi (`kimi --prompt` — red, CLI caído, salida vacía).
...
- **No invoques a Kimi hasta que tu rama esté en verde.** Le revisas el trabajo, no el proceso.
